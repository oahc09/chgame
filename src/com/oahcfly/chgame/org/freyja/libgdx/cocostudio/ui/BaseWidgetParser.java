...
public abstract class BaseWidgetParser {
    ...
    public abstract Actor parse(CocoStudioUIEditor editor, CCWidget widget, CCOption option);
    ...
    public Actor commonParse(CocoStudioUIEditor editor, CCWidget widget, CCOption option, Group parent, Actor actor) {
        ...
        // set origin
        actor.setOrigin(option.getAnchorPointX() * actor.getWidth(), option.getAnchorPointY() * actor.getHeight());

        if (parent == null) {
            actor.setPosition(option.getX() - actor.getOriginX(), option.getY() - actor.getOriginY());
        } else {

            //      锚点要算上父控件的锚点,也就是原点
            actor.setX(parent.getOriginX() - (actor.getOriginX() - option.getX()));

            actor.setY(parent.getOriginY() - (actor.getOriginY() - option.getY()));
        }
        ...
        actor.setColor(option.getColorR() / 255f, option.getColorG() / 255f, option.getColorB() / 255f,
                option.getOpacity() / 255f);
        actor.setTouchable(option.isTouchAble() ? Touchable.enabled : Touchable.disabled);
    ...
    protected void addActor(CocoStudioUIEditor editor, Actor actor, CCOption option) {

...
import com.oahcfly.chgame.org.freyja.libgdx.cocostudio.ui.model.animation.CCAction;
import com.oahcfly.chgame.org.freyja.libgdx.cocostudio.ui.model.animation.CCActionFrame;
import com.oahcfly.chgame.org.freyja.libgdx.cocostudio.ui.model.animation.CCActionNode;
import com.oahcfly.chgame.org.freyja.libgdx.cocostudio.ui.model.animation.CCAnimation;
        ...
        for (CCAction action : animation.getActionlist()) {
            ...
            for (CCActionNode node : nodes) {
                ...
                for (CCActionFrame frame : frames) {
                    Interpolation interpolation = getInterpolation(frame.getTweenType());
                    ...
                    if (length != -1) {
                        duration = Float.parseFloat(frame.getStarttime().substring(0, length));
                    } else {
                        duration = Float.parseFloat(frame.getStarttime());
                    }

                    Action moveTo = Actions.moveTo(frame.getPositionx(), frame.getPositiony(), duration, interpolation);

                    Action scaleTo = Actions.scaleTo(frame.getScalex(), frame.getScaley(), duration, interpolation);

                    Action color = Actions.color(new Color(frame.getColorr() / 255.0f, frame.getColorg() / 255.0f,
                            frame.getColorb() / 255.0f, frame.getOpacity() / 255.0f), duration, interpolation);

                    Action rotateTo = Actions.rotateTo(frame.getRotation(), duration, interpolation);

                    sequenceAction.addAction(Actions.parallel(moveTo, scaleTo, color, rotateTo));
                }
